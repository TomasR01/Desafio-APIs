...
const selectorOpt = async () => {
    try {
        ...
        <option value="${currencies.bitcoin.codigo}">${currencies.bitcoin.nombre}</option>
        `
...
btn.addEventListener("click", async () => {
    if (inputCLP.value === "") {
        ...
        try {
            ...
            result.innerHTML = currencies[selector.value].codigo === "dolar"? `Resultado: $${newValue.toLocaleString('es')} USD`: `Resultado: ${newValue.toLocaleString('es')} UF` ; `Resultado: ${newValue.toLocaleString('es')} BITCOIN`
            renderGrafica()
        } catch(e) {
            result.innerHTML = `¡Hubo un error! Intenta más tarde: ${e.message}`
        }
